...
import com.kstopa.projectmanagement.core.task.{InsertTaskEntity, TaskComment, TaskId, TaskServiceImpl, TaskVolume, UpdateTaskEntity}
        ...
        for {
          ...
          insertionResult <- taskService.insert(user)(
            InsertTaskEntity(
              ...
              createProjectDTO.comment.map(TaskComment),
              createProjectDTO.volume.map(TaskVolume),
            )
          )
        ...
        for {
          ...
          update <- taskService.update(user)(TaskId(taskId))(
            UpdateTaskEntity(
              ...
              updateTaskDTO.comment.map(TaskComment),
              updateTaskDTO.volume.map(TaskVolume),
            )
          )
